...
@section('title', $cat->name . ' | ' . $ws->website_title)
...
<div role="main" class="main shop cp-product-listing-page">
        ...
        <div class="container">
            <div class="row">
                <div class="col-md-12 align-self-center p-static order-2 text-center">
                    <h1 class="text-dark font-weight-bold text-8">{{ $cat->name }}</h1>
                    @if(!empty(trim(strip_tags($cat->excerpt ?? ''))))
                        <span class="sub-title text-dark d-block mt-2">{{ \Illuminate\Support\Str::limit(strip_tags($cat->excerpt), 160) }}</span>
                    @else
                        <span class="sub-title text-dark">Browse products in this category</span>
                    @endif
                </div>
    ...
    @if($cat->fi() && $cat->fi() !== 'not_found.jpg')
        <div class="container-fluid px-0">
            <div class="img-thumbnail border-0 p-0 d-block rounded-0">
                <img class="img-fluid w-100 border-radius-0" src="{{ route('imagecache', ['template' => 'original', 'filename' => $cat->fi()]) }}" alt="{{ $cat->name }}">
            </div>
        </div>
    @endif
        ...
        <div class="row">
                ...
                <aside class="sidebar">
                    <form action="{{ route('searchProduct') }}" method="get">
                        <div class="input-group mb-3 pb-1">
                            <input class="form-control text-1" placeholder="Search products…" name="search" id="productCatSearch" type="text" value="{{ request('search') }}">
                    ...
                    <ul class="nav nav-list flex-column mb-4 mb-lg-5">
                        <li class="nav-item">
                            <a class="nav-link active" href="{{ route('productCategory', ['cat' => $cat->id, 'slug' => $cat->slug ?? '']) }}">All — {{ $cat->name }}</a>
                        </li>
                        @foreach ($subcats as $subcat)
                            <li class="nav-item">
                                <a class="nav-link" href="{{ route('productSubCategory', ['subcat' => $subcat->id, 'slug' => $subcat->slug ?? '']) }}">
                                    {{ $subcat->name }}
                                    @if(isset($subcat->products_count))
            ...
            <div class="col-lg-9 order-1 order-lg-2">
                <div class="cp-category-products-panel">
                    ...
                    @empty
                        <p class="text-center text-muted py-5 mb-0">No products in this category yet.</p>
                    @endforelse
                    </div>
                </div>
...
/* Override default.css .pagination … !important (blue) on this page only */
